...
- (instancetype)initWithParentView:(UIView *)view pickView:(UIView *)pcikView;
{
    self = [super init];
    if (self) {
        [self setup];
        [self configureWithParentLayer:view pickView:pcikView];
    }
    return self;
}
- (void)setup
...
- (void)configureWithParentLayer:(UIView *)parent pickView:(UIView *)pcikView;
{
    if (!parent) {
        return;
    }
    
    self.previewLayer.frame = pcikView.frame;
